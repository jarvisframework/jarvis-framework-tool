...
public class DESede extends SymmetricCrypto {
     ...
     */
    public DESede(ModeEnum mode, PaddingEnum padding) {
     ...
     * @param key 密钥，长度24位
     */
    public DESede(ModeEnum mode, PaddingEnum padding, byte[] key) {
     ...
     * @since 3.3.0
     */
    public DESede(ModeEnum mode, PaddingEnum padding, byte[] key, byte[] iv) {
     ...
     * @since 3.3.0
     */
    public DESede(ModeEnum mode, PaddingEnum padding, SecretKey key) {
        this(mode, padding, key, null);
    }
     ...
     * @since 3.3.0
     */
    public DESede(ModeEnum mode, PaddingEnum padding, SecretKey key, IvParameterSpec iv) {
        this(mode.name(), padding.name(), key, iv);
    }
    ...
    public DESede(String mode, String padding, byte[] key, byte[] iv) {
        this(mode, padding, SecureUtils.generateKey(SymmetricAlgorithmEnum.DESede.getValue(), key), null == iv ? null : new IvParameterSpec(iv));
    }
    ...
    public DESede(String mode, String padding, SecretKey key, IvParameterSpec iv) {
        super(StringUtils.format("{}/{}/{}", SymmetricAlgorithmEnum.DESede.getValue(), mode, padding), key, iv);
    }
    // ------------------------------------------------------------------------- Constructor end
}
