...
public class Digester implements Serializable {
     ...
     */
    public Digester(DigestAlgorithmEnum algorithm) {
     ...
     * @since 4.5.1
     */
    public Digester(DigestAlgorithmEnum algorithm, Provider provider) {
    ...
    public Digester init(String algorithm, Provider provider) {
        if(null == provider) {
            this.digest = SecureUtils.createMessageDigest(algorithm);
        }else {
            try {
