...
import com.jarvisframework.tool.ctypto.BCUtils;
import com.jarvisframework.tool.ctypto.CryptoException;
import com.jarvisframework.tool.ctypto.KeyUtils;
...
public class SM2 extends AbstractAsymmetricCrypto<SM2> {
    ...
    public SM2(byte[] privateKey, byte[] publicKey) {
        this(//
                KeyUtils.generatePrivateKey(ALGORITHM_SM2, privateKey),
                KeyUtils.generatePublicKey(ALGORITHM_SM2, publicKey)
        );
    }
     ...
     */
    @Override
    public byte[] encrypt(byte[] data, KeyTypeEnum keyType) throws CryptoException {
        if (KeyTypeEnum.PublicKey != keyType) {
     ...
     */
    @Override
    public byte[] decrypt(byte[] data, KeyTypeEnum keyType) throws CryptoException {
        if (KeyTypeEnum.PrivateKey != keyType) {
    ...
    public byte[] sign(byte[] data, byte[] id) {
        ...
        try {
            CipherParameters param = new ParametersWithRandom(getCipherParameters(KeyTypeEnum.PrivateKey));
            if (id != null) {
    ...
    public boolean verify(byte[] data, byte[] sign, byte[] id) {
        ...
        final SM2Signer signer = getSigner();
        try {
            CipherParameters param = getCipherParameters(KeyTypeEnum.PublicKey);
            if (id != null) {
                param = new ParametersWithID(param, id);
            }
     ...
     */
    private CipherParameters getCipherParameters(KeyTypeEnum keyType) {
        switch (keyType) {
            case PublicKey:
