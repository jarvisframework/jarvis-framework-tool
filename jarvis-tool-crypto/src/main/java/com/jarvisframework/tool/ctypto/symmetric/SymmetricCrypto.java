...
public class SymmetricCrypto implements Serializable {
     ...
     * 构造
     *
     * @param algorithm 算法 {@link SymmetricAlgorithmEnum}
     * @param key       自定义KEY
     */
     ...
     * @since 3.1.2
     */
    public SymmetricCrypto(SymmetricAlgorithmEnum algorithm, SecretKey key) {
        this(algorithm.getValue(), key);
    }
    ...
    public SymmetricCrypto init(String algorithm, SecretKey key) {
        ...
        if (algorithm.contains(PaddingEnum.ZeroPadding.name())) {
            algorithm = StringUtils.replace(algorithm, PaddingEnum.ZeroPadding.name(), PaddingEnum.NoPadding.name());
            this.isZeroPadding = true;
        }

        this.cipher = SecureUtils.createCipher(algorithm);
        return this;
    }
    ...
    public byte[] encrypt(String data) {
        return encrypt(StringUtils.bytes(data, CharsetUtils.CHARSET_UTF_8));
    }
     ...
     *
     * <p>
     * 在{@link PaddingEnum#ZeroPadding} 模式下，且数据长度不是blockSize的整数倍才有效，否则返回原数据
     *
     * <p>
