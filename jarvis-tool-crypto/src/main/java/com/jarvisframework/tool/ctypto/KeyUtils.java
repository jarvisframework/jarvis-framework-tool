...
import com.jarvisframework.tool.ctypto.symmetric.GlobalBouncyCastleProviderEnum;
...
public class KeyUtils {
    ...
    public static SecretKey generateKey(String algorithm, byte[] key) {
        ...
        if (algorithm.startsWith("PBE")) {
            // PBE密钥
            secretKey = generatePBEKey(algorithm, (null == key) ? null : StringUtils.str(key, CharsetUtils.CHARSET_UTF_8).toCharArray());
    ...
    public static PrivateKey generateRSAPrivateKey(byte[] key) {
        return generatePrivateKey(AsymmetricAlgorithmEnum.RSA.getValue(), key);
    }
    ...
    public static PublicKey generateRSAPublicKey(byte[] key) {
        return generatePublicKey(AsymmetricAlgorithmEnum.RSA.getValue(), key);
    }
