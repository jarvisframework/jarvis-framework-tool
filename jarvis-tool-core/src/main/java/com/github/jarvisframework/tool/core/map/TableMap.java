...
public class TableMap<K, V> implements Map<K, V>, Iterable<Map.Entry<K, V>>, Serializable {
    ...
    public List<V> getValues(K key) {
        return CollectionUtils.getAny(
                this.values,
                ListUtils.indexOfAll(this.keys, (ele) -> ObjectUtils.equal(ele, key))
        );
    }
    ...
    public List<K> getKeys(V value) {
        return CollectionUtils.getAny(
                this.keys,
                ListUtils.indexOfAll(this.values, (ele) -> ObjectUtils.equal(ele, value))
        );
    }
    ...
    public Set<Map.Entry<K, V>> entrySet() {
        ...
        for (int i = 0; i < size(); i++) {
            hashSet.add(new TableMap.Entry<>(keys.get(i), values.get(i)));
        }
        return hashSet;
    }
    ...
    public Iterator<Map.Entry<K, V>> iterator() {
        return new Iterator<Map.Entry<K, V>>() {
            ...
            public Map.Entry<K, V> next() {
                return new TableMap.Entry<>(keysIter.next(), valuesIter.next());
            }
