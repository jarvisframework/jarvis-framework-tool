...
public class IterUtils {
     ...
     * @return {@link Map}
     * @deprecated 如果对象同时实现Iterable和Iterator接口会产生歧义，请使用CollectionUtils.countMap
     */
    @Deprecated
     ...
     * @since 4.0.4
     * @deprecated 如果对象同时实现Iterable和Iterator接口会产生歧义，请使用CollectionUtils.fieldValueMap
     */
    @Deprecated
    ...
    public static <K, V> Map<K, V> fieldValueMap(Iterator<V> iter, String fieldName) {
        return toMap(iter, new HashMap<>(), (value) -> (K) ReflectUtils.getFieldValue(value, fieldName));
    }
    ...
    public static <K, V> Map<K, V> fieldValueAsMap(Iterator<?> iter, String fieldNameForKey, String fieldNameForValue) {
        return toMap(iter, new HashMap<>(),
                (value) -> (K) ReflectUtils.getFieldValue(value, fieldNameForKey),
                (value) -> (V) ReflectUtils.getFieldValue(value, fieldNameForValue)
        );
    }
     ...
     * @return 连接后的字符串
     * @deprecated 如果对象同时实现Iterable和Iterator接口会产生歧义，请使用CollectionUtils.join
     */
    @Deprecated
    ...
    public static <T, K, V> Map<K, List<V>> toListMap(Iterable<T> iterable, Function<T, K> keyMapper, Function<T, V> valueMapper) {
        return toListMap(MapUtils.newHashMap(), iterable, keyMapper, valueMapper);
    }
    ...
    public static <T, K, V> Map<K, V> toMap(Iterable<T> iterable, Function<T, K> keyMapper, Function<T, V> valueMapper) {
        return toMap(MapUtils.newHashMap(), iterable, keyMapper, valueMapper);
    }
