...
public class MapUtils {
    ...
    public static <K, V> Map<K, List<V>> toListMap(Iterable<? extends Map<K, V>> mapList) {
        ...
        for (Map<K, V> map : mapList) {
            ...
            for (Map.Entry<K, V> entry : entrySet) {
                ...
                if (null == valueList) {
                    valueList = CollectionUtils.newArrayList(entry.getValue());
    ...
    public static <K, V> List<Map<K, V>> toMapList(Map<K, ? extends Iterable<V>> listMap) {
        ...
        do {
            ...
            for (Map.Entry<K, ? extends Iterable<V>> entry : listMap.entrySet()) {
                vList = CollectionUtils.newArrayList(entry.getValue());
    ...
    public static <K, V> Map<K, V> getAny(Map<K, V> map, final K... keys) {
        return filter(map, (Filter<Map.Entry<K, V>>) entry -> ArrayUtils.contains(keys, entry.getKey()));
    }
    ...
    public static <K, V, T extends Map<K, V>> T empty(Class<?> mapClass) {
        ...
        // 不支持空集合的集合类型
        throw new IllegalArgumentException(StringUtils.format("[{}] is not support to get empty!", mapClass));
    }
