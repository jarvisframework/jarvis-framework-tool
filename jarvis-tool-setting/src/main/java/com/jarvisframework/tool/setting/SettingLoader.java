...
import com.jarvisframework.tool.log.Log;
...
public class SettingLoader {
    ...
    public SettingLoader(GroupedMap groupedMap) {
        this(groupedMap, CharsetUtils.CHARSET_UTF_8, false);
    }
    ...
    synchronized public boolean load(InputStream settingStream) throws IOException {
        ...
        try {
            ...
            while (true) {
                ...
                // 记录分组名
                if (StringUtils.isSurround(line, CharUtils.BRACKET_START, CharUtils.BRACKET_END)) {
                    group = line.substring(1, line.length() - 1).trim();
                    continue;
                }

                final String[] keyValue = StringUtils.splitToArray(line, this.assignFlag, 2);
                // 跳过不符合键值规范的行
    ...
    synchronized private void store(PrintWriter writer) {
        for (Entry<String, LinkedHashMap<String, String>> groupEntry : this.groupedMap.entrySet()) {
            writer.println(StringUtils.format("{}{}{}", CharUtils.BRACKET_START, groupEntry.getKey(), CharUtils.BRACKET_END));
            for (Entry<String, String> entry : groupEntry.getValue().entrySet()) {
                writer.println(StringUtils.format("{} {} {}", entry.getKey(), this.assignFlag, entry.getValue()));
            }
        }
    }
    ...
    private String replaceVar(String group, String value) {
        // 找到所有变量标识
        final Set<String> vars = RegexUtils.findAll(varRegex, value, 0, new HashSet<>());
        String key;
        for (String var : vars) {
            key = RegexUtils.get(varRegex, var, 1);
            if (StringUtils.isNotBlank(key)) {
                // 本分组中查找变量名对应的值
                String varValue = this.groupedMap.get(group, key);
                // 跨分组查找
                if (null == varValue) {
                    final List<String> groupAndKey = StringUtils.split(key, CharUtils.DOT, 2);
